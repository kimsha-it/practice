...
async function getPostsBook() {
  const response = await axios.get(`${BASE_URL}/posts/tag/books`);
  console.log(response.data);
}
getPostsBook();
/* 출력 결과
...
// phone 검색 코드
async function getProductsSearchPhone() {
  const response = await axios.get(`${BASE_URL}/products/search?q=phone`);
  console.log(response.data.total);
}
getProductsSearchPhone();
// glass 검색 코드
async function getProductsSearchGlass() {
  const response = await axios.get(`${BASE_URL}/products/search?q=glass`);
  console.log(response.data.total);
}
getProductsSearchGlass();
/* 출력 결과
// phone 검색 시
23
